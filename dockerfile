...
RUN export DEBIAN_FRONTEND=noninteractive && apt update -y && apt-get install -y tzdata \
 ...
 && cd ../ && wget -q https://download.qt.io/official_releases/qt/6.4/6.4.2/single/qt-everywhere-src-6.4.2.tar.xz \
 ...
 && cd ../ && ls && rm -rf qt-everywhere-src-6.4.2 && rm -rf qt-everywhere-src-6.4.2.tar.xz && rm -rf cmake-3.21.7.tar.gz && rm -rf cmake-3.21.7
